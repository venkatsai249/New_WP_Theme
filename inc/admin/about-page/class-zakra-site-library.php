...
class Zakra_Site_Library {
	 ...
	 * @return bool
	 */
	public static function zakra_is_premium_theme_plan() {
	...
	public static function zakra_site_library_page_content() {
		...
		$premium_package = self::zakra_is_premium_theme_plan();
		...
		if ( isset( $demo_packages ) ) :
			...
			foreach ( $demo_packages->demos as $demo_package_demo => $demo_package_data ) {
				...
				if ( isset( $demo_package_data->isPremium ) && ! $premium_package ) {
					$output .= '<span class="premium-demo-banner">';
					$output .= esc_html__( 'Premium', 'zakra' );
				...
				// Import button.
				if ( ( isset( $demo_package_data->isPremium ) && $premium_package ) || ! isset( $demo_package_data->isPremium ) ) {
